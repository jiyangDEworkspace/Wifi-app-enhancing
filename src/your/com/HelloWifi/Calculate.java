...
import your.com.HelloWifi.Activity.HelloWifiActivity;
...
public class Calculate {
	...
	public static Data getSampleData(ArrayList<ArrayList> WifiSum){
		...
		for(int i = 0; i < WifiSum.size(); i ++){
			int[] RSS = new int[WifiSum.get(i).size() - 1];//attention
			for(int j = 0; j < WifiSum.get(i).size() - 1; j ++){
				RSS[j] = (Integer) WifiSum.get(i).get(j + 1);
			}
